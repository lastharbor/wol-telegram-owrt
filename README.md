...
### Порты и протоколы (что реально уходит с роутера)
...
Для прохождения WoL между подсетями или через маршрутизаторы может понадобиться ретранслятор/настройка directed broadcast — это вне логики `etherwake` в этом приложении.
